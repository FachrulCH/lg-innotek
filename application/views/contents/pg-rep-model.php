    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">Report NG Customer by Model</h3>
            </div><!-- /.box-header -->
            <div class="box-body">
                            ...
                            <div class="col-sm-3">
                                <select class="form-control" name="model">
                                    ...
                                    foreach ($list_produk as $produk) {
                                        $active = "";
                                        if ($produk['model'] == @$get_data['model']) {
                                            $active = 'selected=""';
                                        }
                                        echo '<option value="' . $produk['part_no'] . '" ' . $active . '>' . $produk['model'] . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
                ...
                $get = $this->input->get();
                if (count($detail_data) > 0) {
                    echo '<a href="' . base_url('report/filterbymodel?startdate=' . $get['startdate'] . '&enddate=' . $get['enddate'] . '&model=' . $get['model'] . '') . '" target="_blank" class="btn btn-success" id="btn-print"> <i class="fa fa-print"></i> Print</a>';
                } else {

    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">Report NG Customer by Status</h3>
            </div><!-- /.box-header -->
            <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-3">
                                <select class="form-control" name="status">
                                    ...
                                    <?php
                                    foreach ($list_status as $i => $status) {
                                        $active = "";
                                        if ($i == @$get_data['status']) {
                                            $active = 'selected=""';
                                        }
                                        echo '<option value="' . $i . '" ' . $active . '>' . $status . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
                ...
                $get = $this->input->get();
                if (count($detail_data) > 0) {
                    echo '<a href="' . base_url('report/filterbystatus?startdate=' . $get['startdate'] . '&enddate=' . $get['enddate'] . '&status=' . $get['status'] . '') . '" target="_blank" class="btn btn-success" id="btn-print"> <i class="fa fa-print"></i> Print</a>';
                } else {

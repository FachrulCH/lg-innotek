    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">Report NG Customer by Employee</h3>
                </div><!-- /.box-header -->
            <div class="box-body">
                <div class="div-filter">                    
                    <form class="form-horizontal">
                        ...
                        <div class="form-group">
                            <label for="cust-name" class="col-sm-2 control-label">Employee Name</label>
                            <div class="col-sm-3">
                                <select class="form-control" name="employee">
                                    ...
                                    foreach ($list_employee as $employee) {
                                        $active = "";
                                        if ($employee['id'] == @$get_data['employee']) {
                                            $active = 'selected=""';
                                        }
                                        echo '<option value="' . $employee['id'] . '" ' . $active . '>'.$employee['id'].'-' . $employee['name'] . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
                ...
                $get = $this->input->get();
                if (count($detail_data) > 0) {
                    echo '<a href="' . base_url('report/filterbyemployee?startdate=' . $get['startdate'] . '&enddate=' . $get['enddate'] . '&employee=' . $get['employee'] . '') . '" target="_blank" class="btn btn-success" id="btn-print"> <i class="fa fa-print"></i> Print</a>';
                } else {

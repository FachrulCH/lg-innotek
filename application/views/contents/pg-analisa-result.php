        ...
        <div class="box">
            ...
            <div class="box-body">
                    ...
                    <tbody>
                        ...
                        if (count($detail_data) > 0) {
                            foreach ($detail_data as $idx => $detail) {
                                $tgl = (is_null($detail['spr_sub_date'])) ? '(kosong)' : format_tgl($detail['spr_sub_date']);
                                $status_analisa = (empty($detail['sp_file_name'])) ? '<span class="label label-warning"><i class="fa fa-hourglass-1"></i> Not Yet</span>' : '<span class="label label-info"><i class="fa fa-check"></i> OK</span>';
                                echo '<tr data-idx="' . $idx . '">
                                        ...
                                        <td>' . $detail['spr_result'] . '</td>
                                        <td>' . $status_analisa . '</td>
                                        <td>  ';
                                
                                if ($detail['sp_file_name'] !== null) {
                                    echo'<a href="' . base_url() . 'unduh/file/' . $detail['sp_file_name'] . '" class="btn btn-success btn-xs btn-download"><i class="fa fa-download"></i> Download</a> ';
                                    echo '<button class="btn btn-danger btn-xs btn-delete"><i class="fa fa-trash"></i> Delete</button>';
                                } else {
                                    echo'<a href="#" class="btn btn-success btn-xs btn-download" disabled="disabled"><i class="fa fa-download"></i> Download</a> ';
                ...
                <div id="pilihan-aksi" style="display: none">
                    <button class="btn btn-success" id="btn-add"> <i class="fa fa-plus"></i> Upload</button>
                </div>
            </div><!-- /.box-body -->
        </div><!-- /.box -->
    </div><!-- /.col -->
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h2 class="modal-title">Add Result Analisa</h2>
            </div>
            <div class="modal-body">
                <form class="form-horizontal" method="post" action="<?= base_url('analisa/simpanresultsp') ?>" enctype="multipart/form-data">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="inputPassword3" class="col-sm-3 control-label">Result Analisa</label>
                        <div class="col-sm-9">
...
<div class="modal fade" id="mdl-hapus" tabindex="-1" role="dialog" aria-labelledby="mdl-hapus" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure to delete this file and result?</p>
            </div>
            <div class="modal-footer">
                <form action="<?= base_url('analisa/hapusresultsp') ?>" method="post">
